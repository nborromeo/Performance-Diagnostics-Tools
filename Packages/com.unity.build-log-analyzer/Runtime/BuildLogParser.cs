...
namespace Unity.Profiling.BuildLogAnalyzer
{
    public partial class BuildLogParser
    {
        ...
        // progressCallback(progress 0..1) returns true to request cancellation.
        public bool Analyze(string filePath, string outputPath, Func<float, bool> progressCallback = null)
        {
            ...
            {
                if (progressCallback != null && CurrentLine % 200 == 0)
                {
                    if (progressCallback((float)CurrentLine / Lines.Length))
                    {
                        _openMarkers.Clear();
        ...
        public virtual int GetMessageInitIndex(ref string line)
        {
            ...
            if (indexOfStep > 0) return line.IndexOf(']', indexOfStep) + 2;
            
            indexOfStep = line.IndexOf(':');
            if (indexOfStep > 0) return indexOfStep + 1;
            return 0;
