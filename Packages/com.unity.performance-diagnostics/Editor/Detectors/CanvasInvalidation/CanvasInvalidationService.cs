...
namespace PerformanceDiagnostics.Detectors
{
    ...
    public static class CanvasInvalidationService
    {
        ...
        static readonly MethodInfo s_CrossFadeColor5;
        ...
        static readonly FieldInfo  s_NativePtrField;
        ...
        static readonly Dictionary<long, InvalidationEntry> s_DedupMap = new Dictionary<long, InvalidationEntry>();
        ...
        static void LateInitialize()
        {
            if (!IsReflectionReady)
            {
                UnityEngine.Debug.LogWarning(
                    "[PerformanceDiagnostics] Canvas invalidation reflection init failed — " +
            ...
            var performUpdate = typeof(CanvasUpdateRegistry).GetMethod("PerformUpdate", instPriv2);
            ...
            void PatchCR(MethodBase orig, string hookName)
            {
                ...
                if (!MinimalPatcher.TryPatch(orig, hook))
                    UnityEngine.Debug.LogWarning(
                        $"[PerformanceDiagnostics] CanvasRenderer patch failed: {hookName}");
            }
